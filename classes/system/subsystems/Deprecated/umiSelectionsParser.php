<?php
	...
	class umiSelectionsParser implements iUmiSelectionsParser {
		...
		*/
		...
		/**
		 * @desc Выбирает id объектов (umiObject) или елементов иерархии (umiHierarchyElement), соответсвующих указаным
		 *     критериям
		...
		public static function runSelection(umiSelection $selection) {
			...
			$result = $connection->queryResult($sqls['result']);
			...
			if ($notOnlyIdsTaken) {
				...
				foreach ($result as $row) {
					if (umiCount($row) === 4 && $permissions->isAllowedObject($permissions->getUserId(), $row['id'])) {
						$res[] = $row;
						$pageIds[] = $row['id'];
					}
				}
		...
		public static function runSelectionCounts(umiSelection $selection) {
			...
			$connection = ConnectionPool::getInstance()->getConnection();
			$result = $connection->queryResult($sqls['count'], true);
			$result->setFetchType(IQueryResult::FETCH_ROW);
			...
			return $selection->count = (int) $count;
		}
		...
		public static function parseSelection(umiSelection $selection) {
			...
			*/
			...
			$selection->sql_arr_for_mark_used_fields = [];
			...
			*/
			...
			$sz = umiCount($selection->usedContentTables);
			if ($sz > 1) {
				for ($i = 0; $i < $sz - 1; $i++) {
...
SQL;
			...
			if ($selection->optimize_root_search_query
				&& umiCount($selection->getElementTypeConds())
				&& !umiCount(array_positive_values($selection->getHierarchyConds()))
			) {
...
SQL;
		...
		/**
		 * Переводит фильтры в sql
		 * @param umiSelection $selection
		 * @param bool $joinsMode служебный параметр для внутреннего использования
		 */
		public static function makePropPart(umiSelection $selection, $joinsMode = false) {
			$connection = ConnectionPool::getInstance()->getConnection();
			if ($searchStrings = $selection->getSearchStrings()) {
				...
				if (umiCount($elements)) {
					...
					if (is_array($types) && umiCount($types)) {
						...
						if ($hierarchyTypeId == 21 && umiCount($types)) {
							$hierarchyTypeId = array_pop($types);
						}
				...
				foreach ($searchStrings as $searchString) {
					...
					$intCond =
						(is_numeric($searchString)) ? " OR ct.float_val = '{$searchString}' OR ct.int_val = '{$searchString}'" : "";
					$searchConds[] =
						"o.name LIKE '%{$searchString}%' OR ct.varchar_val LIKE '%{$searchString}%' OR ct.text_val LIKE '%{$searchString}%' {$intCond}" .
						$fileFieldsCond;
				}

				$selection->sql_arr_for_and_or_part['where'][] =
					"ct.obj_id = o.id AND (" . implode(" OR ", $searchConds) . ")";
			...
			if ($arr_propconds = $selection->getPropertyConds()) {
				...
				foreach ($arr_propconds as $arr_cond) {
					if ($arr_cond['type'] !== false) {
						$prop_cond[] = $arr_cond;
					}
				}
				unset($arr_propconds);

				if ($sz = umiCount($prop_cond)) {
					...
					for ($i = 0; $i < $sz; $i++) {
						...
						if ($s_type == 'optioned') {
							...
							if (in_array($s_type, ['int', 'float', 'varchar', 'tree', 'rel']) == false) {
								continue;
							}
							$s_type .= "_val";
						}
						...
						if (!$joinsMode) {
							...
							$s_common = $cname . ".field_id = '" . $i_field_id . "'";
						}
						...
						switch ($s_filter_type) {
							...
							case 'equal':
								if ($v_value) {
									...
									$s_values = "'" . (implode("', '", array_map([$connection, 'escape'], $v_value))) . "'";
									$s_next_cond = "(" . $s_common . " AND (" . $s_field . " IN (" . $s_values . ")))";
								} else {
									$v_value = $connection->escape($v_value);
							...
							case 'not_equal':
								...
								$s_next_cond =
									"(" . $s_common . " AND ((" . $s_field . " IS NULL) OR (" . $s_field . " NOT IN (" .
									$s_values .
									"))))";
								break;
							...
							case 'like':
								...
								if (substr($v_value, 0, 1) === '%' || substr($v_value, -1) === '%') {
									$b_need_percents = false;
								}

								$s_value = $connection->escape($v_value);

								$s_next_cond =
									"(" . $s_common . " AND " . $s_field . " LIKE '" . ($b_need_percents ? "%" : "") .
									$s_value .
									($b_need_percents ? "%" : "") . "')";
								break;
							...
							case 'between':
								...
								$s_next_cond =
									"(" . $s_common . " AND " . $s_field . " BETWEEN '" . $f_min . "' AND '" . $f_max .
									"')";
								break;
							...
							case 'null':
								$randomSuffix = rand();
								$s_next_cond = <<<SQL
($s_common AND $s_field IS NULL 
OR NOT EXISTS (SELECT 1 
     FROM $tableName c$randomSuffix
     WHERE c$randomSuffix.obj_id = o.id AND c$randomSuffix.field_id = '$i_field_id'))
SQL;
								break;
					...
					if (umiCount($selection->sql_arr_for_and_or_part)) {
						$selection->sql_cond__need_content = true;
		...
		*/
		...
		private static function makeDistinctKeywords(umiSelection $selection) {
			...
			if ($selection->sql_cond__need_content ||
				($selection->sql_cond__need_hierarchy && $selection->sql_part__perms)) {
		...
		private static function makeTables(umiSelection $selection) {
			...
			if ($selection->sql_cond__need_hierarchy) {
				if ($selection->sql_part__content_tables || $selection->objectTableIsRequired ||
					$selection->needSimplePageFields) {
...
SQL;
		...
		*/
		...
		private static function makeLimitPart(umiSelection $selection) {
			...
			if ($limit_cond !== false) {
				if (is_array($limit_cond) && umiCount($limit_cond) > 1 && is_numeric($limit_cond[0]) &&
					is_numeric($limit_cond[1])) {
...
SQL;
				...
				if (umiCount($HierarchyRelationsConds) > 0) {
		...
		private static function makeElementTypePart(umiSelection $selection) {
			...
			if ($element_type_cond && umiCount($element_type_cond)) {
		...
		private static function makeOwnerPart(umiSelection $selection) {
			...
			if (is_array($owner_cond) && umiCount($owner_cond)) {
		...
		private static function makeObjectsPart(umiSelection $selection) {
			...
			if (is_array($objects_cond) && umiCount($objects_cond)) {
		...
		private static function makeElementsPart(umiSelection $selection) {
			...
			if (is_array($elements_cond) && umiCount($elements_cond)) {
			...
			if ($perms_cond = $selection->getPermissionsConds()) {
				...
				if ($sz = umiCount($perms_cond)) {
					...
					for ($i = 0; $i < $sz; $i++) {
						...
						$selection->sql_part__perms .= "(c3p.owner_id = '" . $perms_cond[$i] .
							"' AND c3p.rel_id = h.id AND c3p.level & '{$permissionsLevel}')";
		...
		private static function makeObjectTypePart(umiSelection $selection) {
			...
			if ($object_type_cond && umiCount($object_type_cond)) {
			...
			if ($order_cond) {
				...
				for ($i = 0; $i < $sz; $i++) {
					if ($native_field = $order_cond[$i]['native_field']) {
						...
						$selection->sql_arr_for_and_or_part['order'][] =
							"{$cname}.obj_id = o.id AND {$cname}.field_id = '{$order_cond[$i]['field_id']}'";

						$selection->sql_order_by .= "{$cname}.{$order_cond[$i]['type']} " .
							(($order_cond[$i]['asc']) ? "ASC" : "DESC");
						if ($i == ($sz - 1)) {
						} else {
							$selection->sql_order_by .= ", ";
						}
		...
		private static function makeNamesPart(umiSelection $selection) {
			$arr_names_parts = [];
			$connection = ConnectionPool::getInstance()->getConnection();
			if (($names_cond = $selection->getNameConds()) && umiCount($names_cond)) {
				foreach ($names_cond as $arr_next_name) {

					$cname = $arr_next_name['value'];

					$b_need_percents = true;
					if (substr($cname, 0, 1) === '%' || substr($cname, -1) === '%') {
						$b_need_percents = false;
					}

					$cname = $connection->escape($cname);

					if ($arr_next_name['type'] == 'exact') {
						$arr_names_parts[] = "o.name = '" . $cname . "'";
					} else {
						$arr_names_parts[] =
							"o.name LIKE '" . ($b_need_percents ? "%" : "") . $cname . ($b_need_percents ? "%" : "") . "'";
					}
					$selection->objectTableIsRequired = true;
				}
			}
			if (umiCount($arr_names_parts)) {
		...
		private static function makePropsAndNames(umiSelection $selection) {
			...
			if (isset($selection->sql_arr_for_and_or_part['where'])) {
				...
				$selection->sql_part__props_and_names =
					" AND (" . implode($s_concat_mode, $selection->sql_arr_for_and_or_part['where']) . ")";
			}

			if (isset($selection->sql_arr_for_and_or_part['order'])) {
				$selection->sql_part__props_and_names .= " AND (" .
					implode(" AND ", $selection->sql_arr_for_and_or_part['order']) .
					")";
			}
		}
		...
		private static function makeHierarchySpecificConds(umiSelection $selection) {
			if ($selection->sql_cond__need_hierarchy == true) {
				...
				if ($active_cond = $selection->getActiveConds()) {
					...
					$selection->sql_part__unactive_cond =
						(cmsController::getInstance()->getCurrentMode() != "admin") ? " AND h.is_active = '1' " : "";
				}
			}
		}
		...
		protected static function chooseContentTableName(umiSelection $selection, $fieldId) {
			...
			if (umiCount($hierarchyTypes) == 0) {
				if (umiCount($objectTypes) == 1) {
					reset($objectTypes);
		...
		public static function getObjectTypeByFieldId($fieldId) {
			return umiObjectTypesCollection::getInstance()->getTypeIdByFieldId($fieldId);
		}
