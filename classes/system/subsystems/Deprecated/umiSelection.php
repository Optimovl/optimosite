<?php
	...
	class umiSelection implements iUmiSelection {
		...
		public $sql_arr_for_mark_used_fields = [];

		public $sql_arr_for_and_or_part = [];
		...
		public function result() {
			return umiSelectionsParser::runSelection($this);
		}
		 ...
		 * @param bool $is_enabled Разрешить фильтрацию (true) или запретить (false)
		 */
		public function setPermissionsFilter($is_enabled = true) {
			...
			if (permissionsCollection::getInstance()->isSv($user_id)) {
		...
		public function setOrderByProperty($field_id, $asc = true) {
			...
			$filter = ["field_id" => $field_id, "asc" => $asc, "type" => $data_type, "native_field" => false];
		...
		public function setOrderByOrd($asc = true) {
			...
			$filter = ["type" => "native", "native_field" => "ord", "asc" => $asc];
		...
		public function setOrderByRand() {
			...
			$filter = ["type" => "native", "native_field" => "rand", "asc" => true];
		...
		public function setOrderByName($asc = true) {
			...
			$filter = ["type" => "native", "native_field" => "name", "asc" => $asc];
		...
		public function setOrderByObjectId($asc = true) {
			...
			$filter = ["type" => "native", "native_field" => "object_id", "asc" => $asc];
		...
		public function addHierarchyFilter($element_id, $depth = 0, $ignoreIsDefault = true) {
			...
			if (umiHierarchy::getInstance()->isExists($element_id) || (is_numeric($element_id) && $element_id == 0)) {
				...
				if (in_array($element_id, $this->hierarchy) === false || $element_id == 0) {
					$this->hierarchy[] = [(int) $element_id, $depth];
				}

				if ($depth > 0) {
					$this->hierarchy[] = [$element_id, $depth];
				}
			} else {
				return false;
			}
		...
		public function addPropertyFilterBetween($field_id, $min, $max) {
			...
			$filter =
				["type" => $data_type, "field_id" => $field_id, "filter_type" => "between", "min" => $min, "max" => $max];
			$this->props[] = $filter;
		}
		...
		public function addPropertyFilterEqual($field_id, $value, $case_insencetive = true) {
			...
			$filter = [
				...
				"filter_type" => "equal",
				"value" => $value,
				"case_insencetive" => $case_insencetive
			];
			$this->props[] = $filter;
		}
		...
		public function addPropertyFilterMore($field_id, $value) {
			...
			$filter = ["type" => $data_type, "field_id" => $field_id, "filter_type" => "more", "value" => $value];
			$this->props[] = $filter;
		}
		...
		public function addPropertyFilterLess($field_id, $value) {
			...
			$filter = ["type" => $data_type, "field_id" => $field_id, "filter_type" => "less", "value" => $value];
			$this->props[] = $filter;
		}
		...
		public function addPropertyFilterIsNull($field_id) {
			...
			$filter = ["type" => $data_type, "field_id" => $field_id, "filter_type" => "null"];
			$this->props[] = $filter;
		}
		...
		public function addPropertyFilterIsNotNull($field_id) {
			...
			$filter = ["type" => $data_type, "field_id" => $field_id, "filter_type" => "notnull"];
			$this->props[] = $filter;
		}
		...
		public function getOwnerConds() {
			$arrAnswer = [];
			if (is_array($this->owner) && umiCount($this->owner)) {
		...
		public function getObjectsConds() {
			$arrAnswer = [];
			if (is_array($this->objects_ids) && umiCount($this->objects_ids)) {
		...
		public function getElementsConds() {
			$arrAnswer = [];
			if (is_array($this->elements_ids) && umiCount($this->elements_ids)) {
		...
		public function getElementTypeConds() {
			...
			if ($this->optimize_root_search_query) {
				if (is_array($this->element_type)) {
					if (umiCount($this->element_type) > 1) {
						reset($this->element_type);
						$this->element_type = [current($this->element_type)];
					}
				}
			}
