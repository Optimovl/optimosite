<?php
	...
	class umiXmlImporter implements iUmiXmlImporter {
		...
		protected function analyzeElementNode(SimpleXMLElement $elementNode) {
			...
			$element_is_visible =
				is_object($elementNode->attributes()->is_visible) ? (string) $elementNode->attributes()->is_visible : null;
			$element_is_active =
				is_object($elementNode->attributes()->is_active) ? (string) $elementNode->attributes()->is_active : null;
			$element_is_deleted =
				is_object($elementNode->attributes()->is_deleted) ? (string) $elementNode->attributes()->is_deleted : null;
			...
			$element_hierarchy_type =
				umiHierarchyTypesCollection::getInstance()->getTypeByName((string) $module, (string) $method);
			...
			$this->xml_objects[$element_object_id] = [
				"old_element_id" => $element_id,
		...
		protected function analyzeObjectPropertiesBlockNode(
			SimpleXMLElement $object_properties_block_nodes,
			$object_type_id
		) {
			if (!array_key_exists($object_type_id, $this->xml_types)) {
				$this->xml_types[$object_type_id] = [];
				$this->xml_types[$object_type_id]['is_base'] = true;
				$this->xml_types[$object_type_id]['props'] = [];
			}
		...
		protected function analyzeObjectSoresInfo($iObjectId, SimpleXMLElement $oObjectStoresInfoNodes) {
			foreach ($oObjectStoresInfoNodes as $oStoreInfoXml) {
				...
				if (strlen($sStoreId)) {
					$sStoreId = $sStoreId;
					if (!isset($this->xml_stores[$iObjectId])) {
						$this->xml_stores[$iObjectId] = [];
					}
					$arrStoreInfo = [];
		...
		protected function detectBetterObjectType($hierarchy_type_id, $old_type_id, $new_type_name = "") {
			...
			$base_type_id = umiObjectTypesCollection::getInstance()->getTypeIdByHierarchyTypeId($hierarchy_type_id);
			...
			if (!$new_type_id) {
				...
				if (!$new_type_name || !strlen($new_type_name)) {
					$new_type_name = "Подтип \"{$base_type_name}\" #{$old_type_id}";
				}
		...
		public function importXml() {
			foreach ($this->xml_elements as $element_info) {
				...
				$element_info['new_type_id'] =
				$new_type_id = $this->detectBetterObjectType($hierarchy_type_id, $old_object_type_id, $type_name);
				$element_info['new_tpl_id'] =
				$new_tpl_id = $this->detectBetterTemplateId($element_info['element_filepath']);
		...
		protected function importElement($element_info) {
			...
			$oEventPoint->addRef("props", $props);
			...
			if ($old_element_parent_id === "0") {
				...
				$new_parent_id =
					umiImportRelations::getInstance()->getNewIdRelation($this->source_id, $old_element_parent_id);
			}
			...
			if ($new_element_id === false && $new_parent_id !== false) {
				...
				$new_element_parent_id =
					umiImportRelations::getInstance()->getNewIdRelation($this->source_id, $old_element_parent_id);
				$new_element_id = umiHierarchy::getInstance()->addElement(
					$new_parent_id,
					$new_hierarchy_type_id,
					$new_name,
					$alt_name,
					$new_type_id,
					$new_domain_id,
				...
				if ($new_element->getObject()->getPropByName($prop_name)) {
					if ($field_type == 'price' && strlen($prop_info['currency_code'])) {
						...
						if ($emarket) {
							...
							if ($currency) {
								$f_price =
									$emarket->formatCurrencyPrice([$price], $emarket->getDefaultCurrency(), $currency);
			...
			foreach ($missed_props as $prop_info) {
				...
				if (!$prop_info['name']) {
					$prop_info['name'] = translit::convert($prop_info['title']);
				}
			...
			// update store info
			$iStoreTypeId = umiObjectTypesCollection::getInstance()->getTypeIdByHierarchyTypeName("eshop", "store");
			$iStoreRelType =
				umiObjectTypesCollection::getInstance()->getTypeIdByHierarchyTypeName("eshop", "store_relation");
		...
		protected function getStoreIdByName($sName) {
			$object_type_id = umiObjectTypesCollection::getInstance()->getTypeIdByHierarchyTypeName("eshop", "store");
		...
		protected function addMissedProps(&$new_element, $missed_props, $old_object_type_id) {
			if (strlen($old_object_type_id)) {
				$object_type_id =
					umiImportRelations::getInstance()->getNewTypeIdRelation($this->source_id, $old_object_type_id);
			} else {
			...
			foreach ($missed_props as $missed_prop) {
				...
				if ($prop_group_block = $object_type->getFieldsGroupByName($prop_block_name)) {
					...
					$prop_group_block_id =
						$object_type->addFieldsGroup($prop_block_name, $prop_block_name, true, $prop_block_is_public);
				...
				$field_id = umiFieldsCollection::getInstance()->addField(
					$missed_prop['name'],
					$missed_prop['title'],
					$field_type_id,
					$missed_prop['is_public'],
					false
				);
				...
				if ($object_type_id) {
					umiImportRelations::getInstance()->setFieldIdRelation(
						$this->source_id,
						$object_type_id,
						$missed_prop['name'],
						$field_id
		...
		public function getAutoGuideId($title) {
			...
			$child_types = $typesCollection->getChildTypeIds($rootGuideId);
		 ...
		 * !Deprecated не используется, т.к. не учитывает ошибки. Следует использовать getCreatedElemensCount,
		 *     getUpdatedElementsCount
		 */
