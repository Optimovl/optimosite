...
abstract class admin_typograf_config {
    ...
    public function typograf_config() {
        ...
        if (!$regedit->getVal('//settings/' . $prefix . 'is_first_launch')) {
            ...
            //$q = l_mysql_query("SELECT id, data_type FROM cms3_object_field_types WHERE data_type IN('text','wysiwyg')");
            $q = l_mysql_query("SELECT id, data_type FROM cms3_object_field_types WHERE data_type = 'wysiwyg'");
            $field_types_ids = '';
            while ($row = mysql_fetch_row($q)) {
                /* $title = '';
                  ...
                  $field_types[] = array('node:text' => $title, 'attribute:id' => $row[0]); */
                $field_types_ids .= $row[0] . ',';
            }
        ...
        $settings = array(
            ...
            //array('multiple_select', $prefix . 'field_types', $field_types)
            ),
        );
        ...
        if ($mode == "do") {
            $params = array(
                ...
                //'string:' . $prefix . 'field_types' => NULL,
                )
            );
            ...
            //$regedit->setVar('//settings/' . $prefix . 'field_types', $params['globals']['string:' . $prefix . 'field_types']);
            ...
            $this->setHeaderLabel("header-config-typograf_config");
        }
